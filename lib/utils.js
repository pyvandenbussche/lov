...
exports.relationSuggest  = function (object) {
    ...
    for (i = 0; i < object.length; i++){
      ...
      if(part['prefix'])html+='<a href=\'/vocabs/'+part['prefix']+'\'>('+part['prefix']+')</a>';
      html+='</div>';
    }
    return html;
}
...
//console.log(comment);
    ...
    for (i = 0; i < comment.length; i++){
      var part = comment[i];
      html+='<div>('+this.dateToYMD(part.createdAt)+')</span> <a href="/agents/'+part.agentId["name"]+'">'+part.agentId.name+'</a>: <span>'+part.body+'</div>';
    }
    return html;
}
